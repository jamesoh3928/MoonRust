...
pub fn parse_functioncall_statement(input: &str) -> ParseResult<Statement> {
    // FunctionCall((PrefixExp, Option<String>))
    map(parse_functioncall, |result| Statement::FunctionCall(result))(input)
}
...
fn parse_do_block(input: &str) -> ParseResult<Statement> {
    // DoBlock(Block)
    map(expression::parse_funcbody, |block| {
        Statement::DoBlock(block.1)
    })(input)
}
...
fn parse_function_decl(input: &str) -> ParseResult<Statement> {
    ...
    map(
        tuple((
            ws(tag("function")),
            ws(identifier),
            preceded(common::parse_parlist, expression::parse_funcbody),
        )),
